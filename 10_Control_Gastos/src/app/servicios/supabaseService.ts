...
export class SupabaseService {
  ...
  //! ======================= Métodos auth =======================
  async registrarEmail(email: string, contrasenia: string){
    const {data: sesion, error } = await this.sb.auth.signUp({email: email, password: contrasenia});
  ...
  console.log(`Buscando en ${tabla}: ${columnaIdent} = "${identificador}"`);
  ...
  // alert(`Data devuelta:' ${data} 'Error:' ${error}`);

  if (error) throw new Error(`Error al buscar en ${tabla}: ${(error as Error).message}`);
...
/* STORAGE               */
/* ===================== */
async eliminarArchivo(bucket: string, path: string): Promise<boolean> {
  const { error } = await this.sb
    .storage
    .from(bucket)
    .remove([path]);
