...
@Component({
  ...
  async alternarOrientacion(): Promise<void> {
  try {
    ...
    await ScreenOrientation.lock({
      orientation: esVertical ? 'landscape' : 'portrait',
    });

  } catch (err) {
    console.warn('No se pudo alternar la orientación', err);
  }
}
  ...
  constructor(){
    ...
    async logout() {
      ...
      this.utilSvc.reproducirSonidoPorDuracion('assets/sonidos/error.m4a', 1000);
